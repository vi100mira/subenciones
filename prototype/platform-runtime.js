(function () {
  ...
  function renderPrivateCoverage() {
    ...
    const sourcesInMonitoring = privateMonitoring.map((item) => ({ ...item, title: item.source || item.organism || item.title, programTitle: item.title }));
    const groups = [
      ["Fuentes privadas y oportunidades", sourcesInMonitoring, "Cada fuente muestra sus oportunidades privadas verificadas. Una línea en seguimiento no se presenta como oportunidad hasta disponer de bases y vigencia oficiales."],
    ...
    const preview = (items) => items.map((item) => `<li>${escapeHtml(item.title)}${item.programTitle ? `<small>Seguimiento: ${escapeHtml(item.programTitle)} · 0 oportunidades verificadas</small>` : ""}<small>${escapeHtml(item.evidenceQuality || "Evidencia pendiente")} · ${escapeHtml(item.provenance?.updatedAt ? date(item.provenance.updatedAt) : "Sin fecha")}</small></li>`).join("") || "<li>Sin registros persistidos.</li>";
