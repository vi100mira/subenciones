(function () {
  ...
  function persistedRecommendationRow(recommendation, opportunity, version) {
    ...
    return {
      ...
      deadlineStatus: version?.deadline_status || "uncertain",
      deadlineConfidence: version?.deadline_confidence || "Baja",
      amount: version?.amount_text || "Importe por comprobar",
      ...
      fit: (recommendation?.reasons_json || []).map((reason) => reason?.text).filter(Boolean),
      risks: (recommendation?.risks_json || []).map((risk) => risk?.text).filter(Boolean),
      evidence: (recommendation?.evidence_json || []).map((entry) => entry?.text || entry?.url || entry).filter((entry) => typeof entry === "string" && entry),
      internalFacts: [],
