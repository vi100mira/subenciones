...
function renderSourceNode(source) {
  ...
  const items = coverage.rows.map((item) => `<button class="source-preview-item" data-grid-opportunity="${item.id}" type="button"><strong>${item.title}</strong><span>${item.source || source.name} Â· ${item.deadline || item.deadlineStatus || "Estado por revisar"}</span></button>`).join("");
...
function matchDashboardState(hasTenantMatch, accounting) {
  ...
  if (hasTenantMatch) {
    ...
    return {
      value: total,
      detail: `${accounting.following} en seguimiento Â· ${accounting.outside} fuera Â· ${accounting.notCurrent} no vigentes Â· ${accounting.unmapped} incidencias`,
  ...
  if (["queued", "running"].includes(runStatus)) return { value: "â€”", detail: "Calculo de encaje en curso", note: "El agente esta trabajando. El panel se actualizara al finalizar." };
  if (loadState === "error") return { value: "â€”", detail: "Encaje temporalmente no disponible", note: window.TENANT_MATCH_LOAD_ERROR || "No se ha podido recuperar el resultado persistido." };
  if (runStatus === "review_required") return { value: "â€”", detail: "Resultado calculado Â· recuperando detalle", note: "Existe un encaje terminado; se estan sincronizando sus recomendaciones." };
  if (loadState === "loading") return { value: "â€”", detail: "Cargando encaje guardado", note: "Comprobando el ultimo resultado de la entidad." };
...
function metricPreview(metric) {
  ...
  const items = rows.map((item) => `<button class="metric-preview-item" data-grid-opportunity="${item.id}" type="button"><strong>${item.title}</strong><span>${item.source || "Oportunidad"} Â· ${item.deadline || item.entityFit?.reason || item.deadlineStatus || "Estado por revisar"}</span></button>`).join("");
...
function renderDashboard() {
  ...
  const values = isPlatform
    ? [
        { label: "Oportunidades publicas", value: "â€”", detail: "Cargando corpus comun de plataforma", rows: [], note: "El recuento global se obtiene del estado de plataforma." },
        { label: "Tenants activos", value: "â€”", detail: "Cargando entidades registradas", rows: [], note: "El detalle se mantiene aislado por tenant." },
        { label: "Asistentes operativos", value: "â€”", detail: "Cargando activaciones por tenant", rows: [], note: "Estado agregado sin exponer datos privados." },
        { label: "Revisiones pendientes", value: "â€”", detail: "Cargando decisiones humanas pendientes", rows: [], note: "Solo se contabilizan revisiones persistidas." }
      ]
    : [
