(function () {
  ...
  function canUse(screen) {
    ...
    if (["knowledge", "workspace"].includes(screen)) return current.role === "entity";
  ...
  function openAreaInfo(key) {
    ...
    const included = (plan().agentKeys || []).includes(key);
    ...
    document.body.insertAdjacentHTML("beforeend", `<div class="modal-backdrop" data-plan-area-modal><article class="modal plan-area-modal" role="dialog" aria-modal="true" aria-labelledby="plan-area-title"><div class="panel-heading"><div><p class="eyebrow">${included ? "Área incluida" : "Área no incluida"}</p><h2 id="plan-area-title"><i data-lucide="${icon}"></i>${title}</h2></div><button class="icon-button" data-plan-area-close type="button" aria-label="Cerrar"><i data-lucide="x"></i></button></div><p class="plan-area-purpose">${info.purpose}</p>${preparationDetail}<div class="plan-area-detail-grid"><section><h3>Qué hace</h3><ul>${info.does.map((item) => `<li><i data-lucide="check"></i>${item}</li>`).join("")}</ul></section><section><h3>Límites</h3><p>${info.limits}</p></section></div>${key === "draft_agent" && included ? '<div class="button-row"><button class="primary-action" data-plan-open-preparation type="button">Gestionar desde Base común</button><button class="ghost-action" data-plan-area-close type="button">Cerrar</button></div>' : '<div class="button-row"><button class="ghost-action" data-plan-area-close type="button">Cerrar</button></div>'}</article></div>`);
    window.lucide?.createIcons();
  }

  function applyMenuPolicy() {
    ensureMonitorNavigation(); ensureMonitorScreen();
    document.body.dataset.documentAgentMode = (session()?.plan?.agentKeys || defaultPlan.agentKeys).includes("draft_agent")
      ? "active" : "read_only";
