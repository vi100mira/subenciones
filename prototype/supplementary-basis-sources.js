(function () {
  ...
  function statusBadge(source) {
    if (source.status === "approved" && source.last_verified_at && !source.last_verification_error) return '<span class="badge safe">Captura técnica verificada</span>';
    if (source.status === "approved" && source.last_verification_error) return '<span class="badge warning">Captura técnica fallida</span>';
    if (source.status === "approved") return '<span class="badge review">Permiso técnico · pendiente de captura</span>';
    if (source.status === "rejected") return '<span class="badge warning">Rechazada</span>';
    return '<span class="badge review">Excepción operativa pendiente</span>';
  }
  function render() {
    ...
    const rows = state.sources.map((source) => {
      ...
      const actions = source.status === "proposed" ? `<div class="button-row"><button class="ghost-action" data-supplementary-source-action="reject" data-source-id="${source.id}" type="button">Bloquear captura</button><button class="primary-action" data-supplementary-source-action="approve" data-source-id="${source.id}" type="button">Permitir captura técnica</button></div>` : "";
