(() => {
  ...
  function renderSourceMap(target, readModel) {
    ...
    if (readModel?.territories?.length) {
      ...
      const nationalStatus = Number(national.indexed || 0) ? "active" : "pending";
      const nationalCard = `<details class="source-node ${nationalStatus}"><summary><span class="source-node-copy"><strong>Ámbito estatal</strong><span>${escapeHtml(national.indexed || 0)} indexadas BDNS · ${escapeHtml(national.openVerified || 0)} abiertas verificadas · ${escapeHtml(national.pendingReview || 0)} pendientes de revisión humana</span></span><b aria-label="Convocatorias estatales indexadas">${escapeHtml(national.indexed || 0)}</b><i data-lucide="chevron-down"></i></summary><div class="source-node-preview"><p><strong>Convocatorias estatales no repartidas por comunidad:</strong> ${escapeHtml(national.indexed || 0)} indexadas · ${escapeHtml(national.openVerified || 0)} abiertas verificadas · ${escapeHtml(national.pendingReview || 0)} pendientes de revisión humana.</p><p>Se muestran separadas para que el mapa territorial no las oculte ni las asigne artificialmente a una comunidad autónoma.</p></div></details>`;
      target.innerHTML = `<details class="source-map-info"><summary><i data-lucide="info"></i><span>Cómo leer este mapa</span></summary><div><p>Actualizado: ${escapeHtml(date(readModel.generatedAt))}. Las cifras proceden del índice canónico BDNS; no dependen de que un conector territorial esté activado.</p><p>Las convocatorias de ámbito estatal aparecen en su propia tarjeta; no se reparten entre comunidades.</p></div></details>${nationalCard}${readModel.territories.map((territory) => {
