...
const files = {
  ...
  bridgeSession: fs.readFileSync("api/local-bridge-session.ts", "utf8"),
  bridgeInventory: fs.readFileSync("api/local-bridge-inventory.ts", "utf8"),
  bridgeClient: fs.readFileSync("scripts/local-bridge/run-folder-inventory.mjs", "utf8"),
  bridgeDelivery: fs.readFileSync("api/local-bridge-delivery-session.ts", "utf8"),
...
const checks = [
  ...
  [files.ingestion.includes('source.kind === "local_simulation"') && files.ingestion.includes("puente local aún no está conectado"), "una carpeta local puede quedar en cola sin un puente conectado"],
  [files.bridgeSession.includes("crypto.randomBytes(32)") && files.bridgeSession.includes("token_hash") && files.bridgeSession.includes("local_bridge.session_issued"), "el puente local no usa una sesión efímera auditable"],
  [files.bridgeSession.includes('source.kind !== "local_simulation"') && files.bridgeSession.includes("externalTransfer !== false"), "la sesión del puente no limita fuente local ni transferencia externa"],
  [files.bridgeInventory.includes("token_hash") && files.bridgeInventory.includes("local_bridge.inventory_completed") && files.bridgeInventory.includes("content_stored_remotely: false"), "el resultado del puente no valida la sesión ni audita el inventario privado"],
  [files.bridgeClient.includes("--session-token") && files.bridgeClient.includes("authorization: `Bridge ${token}`") && !files.bridgeClient.includes("SERVICE_ROLE"), "el cliente local requiere una clave administrativa o no usa sesión efímera"],
  [files.bridgeDelivery.includes("writeBackOutputs !== true") && files.bridgeDelivery.includes("Insertia/Candidaturas") && files.bridgeDelivery.includes("overwrite: false") && files.bridgeDelivery.includes("delete: false"), "la entrega local no exige permiso separado o puede modificar originales"],
  ...
  [files.ui.includes("Conectar el puente local") && files.ui.includes("no se creará una cola"), "la interfaz sugiere que una carpeta local se analiza sin puente"],
