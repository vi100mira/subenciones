...
const files = {
  ...
  demoStatus: fs.readFileSync("api/demo-tenant-status.ts", "utf8"),
...
const checks = [
  ...
  [files.demoStatus.includes('requireSourcePermission(req.headers.authorization, "sources:read"') && files.demoStatus.includes('.eq("id", actor.tenantId)'), "el estado demo expone metadatos del tenant sin autenticación o aislamiento"],
