...
assert(!workflow.includes("schedule:") && !workflow.includes("github.event_name == 'schedule'"), "El cron debe permanecer pausado durante la validación de producción");
assert(workflow.includes("solo se pueden iniciar manualmente desde Actions"), "La pausa del cron no queda explicada en el workflow");
